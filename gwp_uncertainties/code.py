...
def get_uncertain_flows(time_horizon=100):
    ...
    for i,act_name in enumerate(act_names):
        delta_std_multiplier_dict[act_name] = get_delta_std_multiplier(act_name, time_horizon, i+1)
...
compute_agwp_general  = lambda A, tau, H, mult: A*tau*(1-np.exp(-H/tau)) * mult
...
compute_dgwp_dA      = lambda A, tau, H, mult, co2_agwp, delta_A: \
    mult * tau / co2_agwp    * (1 - np.exp(-H/tau)) * delta_A*A
compute_dgwp_dtau    = lambda A, tau, H, mult, co2_agwp, delta_tau: \
...
def compute_delta_std_gwp(A, tau, H, delta_A, delta_tau, mult=1, *dgwp_dy):
    co2_delta_agwp, co2_std_agwp, co2_agwp = compute_co2_agwp(H)
    x_agwp         = compute_agwp_general (A, tau, H, mult)
    x_gwp          = compute_gwp(x_agwp, co2_agwp)
    x_dgwp_dA      = compute_dgwp_dA  (A, tau, H, mult, co2_agwp, delta_A)
    x_dgwp_dtau    = compute_dgwp_dtau(A, tau, H, mult, co2_agwp, delta_tau)
...
def get_delta_std_multiplier(substance_name, time_horizon, i='-->'):
    try:
        if "Methane" in substance_name and 'non-fossil' not in substance_name:
        ...
        delta_tau = dict_.get("Lifetime delta", 0.1)
        delta_A = dict_.get("Radiative efficiency delta", 0.2)
        mult = 1
        if 'Methane' in substance_name:
            ...
            x_delta_gwp, x_std_multiplier_gwp = compute_delta_std_gwp(
                A, tau, time_horizon, delta_A, delta_tau, mult, ch4_dgwp_df1, ch4_dgwp_df2,
            )
        elif "Dinitrogen monoxide" in substance_name:
            f1 = dict_.get("f1")  # due to effects on ozone
            f2 = dict_.get("f2")  # due to stratospheric H2O
            delta_f1 = dict_.get("delta_f1")
            delta_f2 = dict_.get("delta_f2")
            dict_methane = substances_data[substance_name]
            ...
            x_delta_gwp, x_std_multiplier_gwp = compute_delta_std_gwp(
                A, tau, time_horizon, delta_A, delta_tau, mult, n2o_dgwp_df1, n2o_dgwp_df2
            )
        else:
            x_delta_gwp, x_std_multiplier_gwp = compute_delta_std_gwp(
                A, tau, time_horizon, delta_A, delta_tau, mult,
            )
        print_uncertainties(i, substance_name, x_delta_gwp, x_std_multiplier_gwp)
        return x_std_multiplier_gwp
    except:
